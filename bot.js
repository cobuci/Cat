...
Actions.checkConditions = function (msg, cmd) {
  ...
  switch (restriction) {
    ...
    case 4:
      return (
        Files.data.settings.ownerId &&
        msg.author.id === Files.data.settings.ownerId
      );
    default:
      return true;
  }
...
Audio.playFile = function (url, options, id) {
  this.dispatchers[id] = this.connections[id].play(
    Actions.getLocalFile(url),
    options
  );
  return true;
};
      ...
      getDefaultChannel() {
        ...
        if (!channel) {
          this.channels.cache.array().forEach(function (c) {
            if (
              c.permissionsFor(DBM.Bot.bot.user).has("SEND_MESSAGES") &&
              c.type !== "voice" &&
              c.type !== "category"
            ) {
              if (!channel) {
